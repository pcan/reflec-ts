...
`/**
  ...
  */`);
        ...
        describe("getFirstToken", () => {
            it("gets jsdoc", () => {
                const root = ts.createSourceFile("foo.ts", "/** comment */var a = true;", { target: ts.ScriptTarget.ES5 }, /*setParentNodes*/ true);
        ...
        describe("getLastToken", () => {
            it("gets jsdoc", () => {
                const root = ts.createSourceFile("foo.ts", "var a = true;/** comment */", { target: ts.ScriptTarget.ES5 }, /*setParentNodes*/ true);
                assert.isDefined(root);
