...
namespace Harness {
    ...
    export namespace Compiler {
        ...
        export function getDefaultLibrarySourceFile(fileName = defaultLibFileName): ts.SourceFile {
            ...
            if (!sourceFile) {
                libFileNameSourceFileMap.set(fileName, sourceFile = createSourceFileAndAssertInvariants(fileName, IO.readFile(libFolder + fileName), { target: ts.ScriptTarget.Latest }));
            }
            return sourceFile;
        }
                ...
                if (file.content !== undefined) {
                    ...
                    if (file.fileOptions && file.fileOptions["symlink"]) {
                        ...
                        for (const link of links) {
                            ...
                            const sourceFile = createSourceFileAndAssertInvariants(linkPath, file.content, options);
